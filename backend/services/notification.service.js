...
// Notifier l'admin qu'une preuve de paiement a été soumise
async function sendPaymentProofNotificationToAdmin(user, event, payment) {
  try {
    // Utiliser le mécanisme d'email existant (Brevo/Nodemailer)
    const adminEmail = process.env.ADMIN_EMAIL;
    const subject = `[Smart Invite] Nouvelle preuve de paiement — ${event.title}`;
    const html = `
      <h2>Nouvelle preuve de paiement soumise</h2>
      <p><strong>Organisateur :</strong> ${user.name} (${user.email})</p>
      <p><strong>Événement :</strong> ${event.title}</p>
      <p><strong>Quota :</strong> ${payment.quota} invités</p>
      <p><strong>Montant :</strong> ${payment.amount} XAF</p>
      <p><strong>Référence :</strong> ${payment.proof_reference || 'Non renseignée'}</p>
      <p><strong>Preuve :</strong> <a href="${payment.proof_url}">Consulter la preuve</a></p>
      <p>Connectez-vous au dashboard admin pour valider ou rejeter ce paiement.</p>
    `;
    await sendEmail(adminEmail, subject, html); // utiliser la fonction d'envoi existante
  } catch (err) {
    console.error('[sendPaymentProofNotificationToAdmin] Erreur:', err.message);
  }
}

// Notifier l'utilisateur du statut de son paiement
async function sendPaymentStatusToUser(user, event, status, reason) {
  const adminEmail = process.env.ADMIN_EMAIL;
  const brevo = new Brevo.TransactionalEmailsApi();
  const logo = await getLogoUrlFromFirebase('logo.png');
  brevo.authentications['apiKey'].apiKey = process.env.BREVO_API_KEY?.trim();
  try {
    const subject = status === 'validated'
      ? `[Smart Invite] Paiement validé — ${event.title}`
      : `[Smart Invite] Paiement refusé — ${event.title}`;
            ...
            <table width="600" cellpadding="0" cellspacing="0"
                  ...
                  ${
                    status === 'validated'
                      ? `
                        ...
                        <p>
                          Nous avons le plaisir de vous informer que votre paiement
                          pour l'événement <strong>${event.title}</strong>
                          a été validé avec succès.
                        </p>

                        <p>
                          Vous pouvez désormais accéder à toutes les fonctionnalités
                          associées à votre événement et générer vos liens d'invitation.
                        </p>
                      ...
                      : `
                        ...
                        <p>
                          Après vérification, nous ne pouvons malheureusement pas
                          valider votre preuve de paiement pour l'événement
                        ...
                        <p>
                          Nous vous invitons à soumettre une nouvelle preuve de
                          paiement afin que votre demande puisse être réexaminée.
                        </p>
              ...
              <tr>
                <td style="
                  ...
                  color:#ffffff;
                ">
                  © ${new Date().getFullYear()} SmartInvite.
    ...
    console.error('[sendPaymentStatusToUser] Erreur:', err.message);
  }
}
...
module.exports = {
  ...
  sendPdfToGuestMail, 
  sendNewsUpdatesToUsers, 
  sendPaymentProofNotificationToAdmin, 
  sendPaymentStatusToUser,
